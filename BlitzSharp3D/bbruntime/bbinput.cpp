...
int bbJoyPitchDir(int port) {
	if (port < 0 || port >= (int)gx_joysticks.size()) return 0;
	float t = gx_joysticks[port]->getAxisState(5);
	return t < JLT ? -1 : (t > JHT ? 1 : 0);
}

int bbJoyYawDir(int port) {
	if (port < 0 || port >= (int)gx_joysticks.size()) return 0;
	float t = gx_joysticks[port]->getAxisState(6);
	return t < JLT ? -1 : (t > JHT ? 1 : 0);
}

int bbJoyRollDir(int port) {
	if (port < 0 || port >= (int)gx_joysticks.size()) return 0;
	float t = gx_joysticks[port]->getAxisState(7);
	return t < JLT ? -1 : (t > JHT ? 1 : 0);
}
...
void input_link(void (*rtSym)(const char* sym, void* pc)) {
	...
	rtSym("%JoyPitchDir%port=0", bbJoyPitchDir);
	rtSym("%JoyYawDir%port=0", bbJoyYawDir);
	rtSym("%JoyRollDir%port=0", bbJoyRollDir);
