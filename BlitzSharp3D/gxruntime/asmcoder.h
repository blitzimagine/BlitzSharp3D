...
public:
	...
	{
		Code(0x8a); if (disp == 0 && src != ebp) { Code((dest << 3) | (src)); }
		else { Code((0x40) | (dest << 3) | (src)); Code(disp); }
	}
	...
	{
		Code(0x88);
		if (disp == 0 && dest != ebp) { Code((src << 3) | (dest)); Code(0); }
		else { Code((0x40) | (src << 3) | (dest)); Code(disp); }
	}
	...
	void shift(Reg32 reg, int imm)	//+imm=left -imm=right
	{
		int		op;
		if (imm == 0) return;
		if (imm > 0) { op = 0xe0 + reg; }
		else { op = 0xe8 + reg; imm = -imm; }
	...
	void and(Reg32 reg, int imm)
	{
		...
		{
			if (reg == eax) { Code(0x25); }
			else { Code(0x81); Code(0xe0 + reg); }Code32(imm);
		}
	}
	void or (Reg32 reg, int imm)
	{
		...
		{
			if (reg == eax) { Code(0x0d); }
			else { Code(0x81); Code(0xc0 + reg); }
			Code32(imm);
		}
	}
