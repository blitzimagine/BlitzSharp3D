...
bool MeshCollider::collide( const Box &line_box,const Line &line,float radius,const Transform &tform,Collision *curr_coll,MeshCollider::Node *node ){
	...
	for( int k=0;k<(int)node->triangles.size();++k ){
...
bool MeshCollider::intersects( const MeshCollider &c,const Transform &t )const{
	...
	for( int k=0;k<(int)leaves.size();++k ){
		...
		Box box=t*p->box;
		bool tformed=false;
		for( int j=0;j<(int)c.leaves.size();++j ){
			Node *q=c.leaves[j];
			if( !box.overlaps( q->box ) ) continue;
			if( !tformed ){
				for( int n=0;n<(int)p->triangles.size();++n ){
			...
			for( int n=0;n<(int)q->triangles.size();++n ){
				...
				for( int t=0;t<(int)p->triangles.size();++t ){
